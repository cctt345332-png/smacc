...
export default function StoreSettingsPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const quickLinks = [
    { href: `/${locale}/inventory/items`,      labelAr: "إدارة المنتجات", labelEn: "Manage Products", icon: <IcPackage />,      color: "#2563EB", bg: "#EFF6FF" },
    { href: `/${locale}/sales/orders`,         labelAr: "الطلبات",         labelEn: "Orders",           icon: <IcShoppingCart />, color: "#059669", bg: "#ECFDF5" },
    { href: `/${locale}/inventory/categories`, labelAr: "التصنيفات",       labelEn: "Categories",       icon: <IcTag />,          color: "#7C3AED", bg: "#F5F3FF" },
  ];
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {storeEnabled && (
            <div style={{ padding: "14px 18px", background: "linear-gradient(135deg, #EFF6FF, #F5F3FF)", borderRadius: 12, border: "1px solid #BFDBFE", display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ color: "#2563EB" }}><IcGlobe /></span>
              <div style={{ flex: 1 }}>
              ...
              <a href={`https://smacc.sa/store/${storeSlug}`} target="_blank" rel="noopener noreferrer" className="btn btn-sm" style={{ background: "#2563EB", color: "white" }}>
                {ar ? "فتح" : "Open"}
              </a>
            </div>
          )}
