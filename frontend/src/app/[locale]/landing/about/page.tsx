...
export default function AboutPage() {
  ...
  const values = [
    { icon: <IcShield />, color: "#2563EB", bg: "#EFF6FF", title: ar ? "الامتثال أولاً" : "Compliance First",    desc: ar ? "نظامنا مبني من الأساس ليكون متوافقاً مع متطلبات زاتكا والفوترة الإلكترونية المرحلة الثانية" : "Our system is built from the ground up to comply with ZATCA Phase 2 e-invoicing requirements" },
    ...
    { icon: <IcUsers />,  color: "#7C3AED", bg: "#F5F3FF", title: ar ? "سهولة الاستخدام" : "Ease of Use",         desc: ar ? "واجهة عربية أولاً، مصممة لتكون بسيطة وسريعة حتى بدون خبرة محاسبية" : "Arabic-first interface, designed to be simple and fast even without accounting experience" },
  ];
    ...
    <div dir={ar ? "rtl" : "ltr"} style={{ fontFamily: "'Alexandria', sans-serif", color: "#0F172A", background: "#fff", minHeight: "100vh" }}>
      ...
      <section style={{ padding: "32px 5%", background: "#0F172A" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 24, textAlign: "center" }}>
          {stats.map((s, i) => (
            <div key={i}>
              <div style={{ fontSize: 32, fontWeight: 800, color: "#2563EB", marginBottom: 4 }}>{s.value}</div>
      ...
      {/* CTA */}
      <section style={{ padding: "48px 5%", background: "#2563EB", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <Link href={`/${locale}/register`} style={{ padding: "12px 28px", borderRadius: 10, background: "white", color: "#2563EB", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
