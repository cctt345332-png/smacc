          ...
          {selectedAcc && (
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
              {[
                ...
                { label: ar ? "إجمالي المدين" : "Total Debit", value: `${fmt(totalDebit)} ${ar ? "ر.س" : "SAR"}`, color: "#5A187E" },
                { label: ar ? "إجمالي الدائن" : "Total Credit", value: `${fmt(totalCredit)} ${ar ? "ر.س" : "SAR"}`, color: "#6F4A84" },
                ...
                <table>
                  ...
                  <tbody>
                    {rows.map((r, i) => (
                      <tr key={i}>
                        ...
                        <td style={{ textAlign: "end", color: "#5A187E", fontWeight: Number(r.debit) > 0 ? 600 : 400 }}>{Number(r.debit) > 0 ? fmt(Number(r.debit)) : "—"}</td>
                        <td style={{ textAlign: "end", color: "#6F4A84", fontWeight: Number(r.credit) > 0 ? 600 : 400 }}>{Number(r.credit) > 0 ? fmt(Number(r.credit)) : "—"}</td>
                  ...
                  <tfoot>
                    <tr style={{ background: "#F8FAFC", borderTop: "2px solid var(--border)", fontWeight: 700 }}>
                      ...
                      <td style={{ textAlign: "end", padding: "12px 16px", color: "#5A187E" }}>{fmt(totalDebit)}</td>
                      <td style={{ textAlign: "end", padding: "12px 16px", color: "#6F4A84" }}>{fmt(totalCredit)}</td>
