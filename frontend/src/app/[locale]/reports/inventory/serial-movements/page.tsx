...
export default function SerialMovementsReportPage(props: { params: Promise<{ locale: string }> }) {
  ...
  useEffect(() => { load(); }, [warehouseId, billId, productId, movementType, fromDate, toDate]);
  ...
  const exportExcel = () => {
    ...
    const exportRows = rows.map((row: any, index: number) => ({
      ...
      [ar ? "حالة السيريال" : "Serial Status"]: row.serial_status || "",
  ...
  const clearFilters = () => {
    setWarehouseId(""); setBillId(""); setProductId(""); setMovementType(""); setFromDate(""); setToDate("");
  };
  ...
  const printRows = rows.map((row: any) => [
    ...
    `${fmt(row.unit_cost)} SAR`, `${fmt(row.total_value)} SAR`, row.bill_number || "—",
  ]);
