...
export default function AgingReportPage(props: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    ...
    {loaded && <div className="card"><div className="card-header"><span className="card-title">{ar ? `تفاصيل أعمار الديون — ${fromDate} إلى ${toDate} — حتى ${asOf}` : `Receivables aging — ${fromDate} to ${toDate} — as of ${asOf}`}</span><span style={{ fontSize: 12, color: "var(--text-secondary)" }}>{ar ? "المسدد من سندات قبض العميل، بما فيها السندات غير المرتبطة بفاتورة" : "Paid from customer receipts, including receipts not linked to an invoice"}</span></div><div className="table-wrapper" style={{ border: "none" }}><table><thead><tr><th>{ar ? "العميل" : "Customer"}</th><th style={{ textAlign: "end" }}>{ar ? "مبلغ الفاتورة" : "Invoice amount"}</th><th>{ar ? "تاريخ الفاتورة" : "Invoice date"}</th><th style={{ textAlign: "end" }}>{ar ? "عدد أيام الفاتورة" : "Invoice days"}</th><th style={{ textAlign: "end" }}>{ar ? "المتبقي من الفاتورة" : "Remaining"}</th><th style={{ textAlign: "end" }}>{ar ? "المسدد" : "Paid"}</th></tr></thead><tbody>{rows.map(r => { const late = r.days > 21; const dayLabel = r.days < 0 ? `${Math.abs(r.days)} ${ar ? "يوم متبقي" : "days remaining"}` : r.days === 0 ? (ar ? "مستحق اليوم" : "Due today") : `${r.days} ${ar ? "يوم متأخر" : "days overdue"}`; return <tr key={r.id} style={{ background: late ? "#FEF2F2" : undefined }}><td style={{ fontWeight: 600 }}>{r.customer}</td><td style={{ textAlign: "end" }}>{fmt(r.sold)}</td><td>{date(r.invoiceDate)}</td><td style={{ textAlign: "end", fontWeight: 800, color: late ? "#DC2626" : r.days > 0 ? "#D97706" : "#15803D" }}>{dayLabel}{late && <span className="badge badge-danger" style={{ marginInlineStart: 6 }}>{ar ? "متأخر" : "Overdue"}</span>}</td><td style={{ textAlign: "end", fontWeight: 700 }}>{fmt(r.balance)}</td><td style={{ textAlign: "end", color: "#15803D" }}>{fmt(r.paid)}</td></tr>; })}</tbody><tfoot><tr style={{ background: "#F8FAFC", fontWeight: 800 }}><td>{ar ? "الإجمالي" : "TOTAL"}</td><td style={{ textAlign: "end" }}>{fmt(totals.sold)}</td><td></td><td></td><td style={{ textAlign: "end" }}>{fmt(totals.balance)}</td><td style={{ textAlign: "end" }}>{fmt(totals.paid)}</td></tr></tfoot></table>{rows.length === 0 && <div className="empty-state"><div className="empty-state-title">{ar ? "لا توجد فواتير مستحقة ضمن الفلاتر" : "No outstanding invoices match the filters"}</div></div>}</div></div>}
  </>;
}
