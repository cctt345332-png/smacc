...
export default function ReceiptDetailPage(props: { params: Promise<{ locale: string; id: string }> }) {
  ...
  if (!voucher) return (
    <div className="empty-state" style={{ minHeight: "60vh" }}>
      ...
      <Link href={`/${locale}/treasury/receipts`} className="btn btn-primary btn-sm" style={{ marginTop: 12 }}>{ar ? "العودة" : "Back"}</Link>
    </div>
  );
    ...
    <>
      <div className="page-header">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn btn-secondary btn-sm" onClick={() => window.open(`/${locale}/treasury/receipts/${id}/print`, "_blank")}>
      ...
      <div className="card">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: "1px solid var(--border)" }}>
          ...
          <div style={{ padding: "18px 28px" }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: "var(--text-muted)", marginBottom: 10, letterSpacing: "0.08em" }}>
              {ar ? "المستلم منه" : "Received From"}
            </div>
            <div style={{ fontSize: 15, fontWeight: 700 }}>{voucher.party_name || "—"}</div>
            {voucher.party_type && (
              <div style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 4 }}>
                {voucher.party_type === "customer" ? (ar ? "عميل" : "Customer") :
                 voucher.party_type === "vendor" ? (ar ? "مورد" : "Vendor") : (ar ? "أخرى" : "Other")}
              </div>
            )}
            {voucher.invoice_id && (
              <div style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 4 }}>
                {ar ? "الفاتورة:" : "Invoice:"}{" "}
                <Link href={`/${locale}/sales/invoices/${voucher.invoice_id}`} style={{ color: "var(--primary)" }}>
