...
export default function EditSalesInvoicePage(props: { params: Promise<{ locale: string; id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([getInvoice(id), getCustomers()]).then(([inv, cust]) => {
      ...
      setLines((x.lines || []).map((l: any) => { const ids = l.serial_ids_json ? JSON.parse(l.serial_ids_json) : (l.serial_item_id ? [l.serial_item_id] : null); const labels = (l.serial_details || []).map((s: any) => s.serial_number || s.id); return { description_ar: l.description_ar || "", quantity: String(l.quantity || 1), unit_price: String(l.unit_price || 0), discount_pct: String(l.discount_pct || 0), vat_rate: String(l.vat_rate ?? 15), inventory_item_id: l.inventory_item_id, serial_item_id: l.serial_item_id, serial_ids: ids, serial_text: labels.length ? labels.join("\n") : (ids || []).join("\n"), variant_id: l.variant_id }; }));
  ...
  return <>
    ...
    <div className="card" style={{ marginTop: 16 }}><div className="card-header"><span className="card-title">{ar ? "أسطر الفاتورة" : "Invoice lines"}</span><button className="btn btn-secondary btn-sm" onClick={() => setLines(x => [...x, emptyLine()])}>+ {ar ? "إضافة سطر" : "Add line"}</button></div><div className="table-wrapper"><table><thead><tr><th>{ar ? "الصنف / الوصف" : "Item / description"}</th><th>{ar ? "الكمية" : "Qty"}</th><th>{ar ? "سعر الوحدة" : "Unit price"}</th><th>{ar ? "الخصم %" : "Discount %"}</th><th>{ar ? "الضريبة %" : "VAT %"}</th><th>{ar ? "السيريالات الحقيقية — كل رقم في سطر" : "Serial numbers — one per line"}</th><th></th></tr></thead><tbody>{lines.map((l, i) => <tr key={i}><td><input className="form-input" value={l.description_ar} onChange={e => setLine(i, "description_ar", e.target.value)} /></td><td><input className="form-input" type="number" min="1" value={l.quantity} onChange={e => setLine(i, "quantity", e.target.value)} /></td><td><input className="form-input" type="number" min="0" step="0.01" value={l.unit_price} onChange={e => setLine(i, "unit_price", e.target.value)} /></td><td><input className="form-input" type="number" min="0" value={l.discount_pct} onChange={e => setLine(i, "discount_pct", e.target.value)} /></td><td><input className="form-input" type="number" min="0" value={l.vat_rate} onChange={e => setLine(i, "vat_rate", e.target.value)} /></td><td><textarea className="form-input" rows={Math.min(6, Math.max(2, (l.serial_text || "").split("\n").length))} placeholder={ar ? "رقم السيريال\nرقم السيريال" : "serial number\nserial number"} value={l.serial_text || ""} onChange={e => { const text = e.target.value; setLines(prev => prev.map((row, n) => n === i ? { ...row, serial_text: text, serial_ids: text.split(/[\n,]+/).map(x => x.trim()).filter(Boolean) } : row)); }} /></td><td><button className="btn btn-ghost btn-sm" onClick={() => setLines(x => x.filter((_, n) => n !== i))} disabled={lines.length === 1}>×</button></td></tr>)}</tbody></table></div></div>
  </>;
}
