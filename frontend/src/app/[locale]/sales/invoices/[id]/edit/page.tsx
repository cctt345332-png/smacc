...
type Line = { description_ar: string; quantity: string; unit_price: string; discount_pct: string; vat_rate: string; inventory_item_id?: string | null; serial_item_id?: string | null; serial_ids?: string[] | null; variant_id?: string | null };
...
export default function EditSalesInvoicePage(props: { params: Promise<{ locale: string; id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([getInvoice(id), getCustomers()]).then(([inv, cust]) => {
      ...
      setLines((x.lines || []).map((l: any) => ({ description_ar: l.description_ar || "", quantity: String(l.quantity || 1), unit_price: String(l.unit_price || 0), discount_pct: String(l.discount_pct || 0), vat_rate: String(l.vat_rate ?? 15), inventory_item_id: l.inventory_item_id, serial_item_id: l.serial_item_id, serial_ids: l.serial_ids_json ? JSON.parse(l.serial_ids_json) : null, variant_id: l.variant_id })));
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await updateInvoice(id, { customer_id: customerId, ...form, issue_date: `${form.issue_date}T00:00:00`, supply_date: `${form.supply_date}T00:00:00`, due_date: form.due_date ? `${form.due_date}T00:00:00` : null, lines: lines.map((l, i) => ({ ...l, line_order: i, quantity: Number(l.quantity), unit_price: Number(l.unit_price), discount_pct: Number(l.discount_pct), vat_rate: Number(l.vat_rate), serial_ids: l.serial_ids || null })) });
