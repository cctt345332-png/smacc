      ...
      {summary && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 20 }}>
          {[
            ...
            { label: ar ? "عدد الفواتير" : "Invoices",       value: summary.invoice_count,             color: "#65707E" },
          ...
          <div className="card" style={{ padding: 20, gridColumn: "1 / -1" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 20, fontWeight: 700, color: "#485668" }}>{rep.target_monthly > 0 ? fmt(rep.target_monthly) + " SAR" : "—"}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 4 }}>{ar ? "نسبة التحقق" : "Achievement"}</div>
                <div style={{ fontSize: 20, fontWeight: 700, color: pct !== null ? (pct >= 100 ? "#059669" : pct >= 70 ? "#D97706" : "#485668") : "var(--text-muted)" }}>
                  {pct !== null ? `${pct}%` : "—"}
                </div>
                {pct !== null && (
                  <div style={{ marginTop: 6, height: 6, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${Math.min(pct, 100)}%`, background: pct >= 100 ? "#059669" : pct >= 70 ? "#D97706" : "#485668", borderRadius: 3, transition: "width 0.3s" }} />
                  </div>
                )}
              </div>
              <div>
                <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 4 }}>{ar ? "نسبة العمولة" : "Commission %"}</div>
                <div style={{ fontSize: 20, fontWeight: 700, color: "#65707E" }}>{rep.commission_pct > 0 ? `${rep.commission_pct}%` : "—"}</div>
...
function TrackingTab({
  ...
  useEffect(() => {
    ...
    const polyline = L.polyline(coords, { color: "#485668", weight: 3, opacity: 0.8 });
    ...
    trackPoints.forEach((p, i) => {
      ...
      const dot = L.circleMarker([p.latitude, p.longitude], { radius: 5, color: "#65707E", fillColor: "#65707E", fillOpacity: 0.7, weight: 1 });
