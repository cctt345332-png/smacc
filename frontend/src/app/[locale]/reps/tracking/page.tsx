  ...
  const drawRoute=useCallback((pts:HistoryPoint[],trips:Trip[],focusIdx:number|null)=>{
    ...
    trips.forEach((trip,i)=>{
      ...
      if(trip.type==="move"&&seg.length>=2){
        const line=L.polyline(seg.map(p=>[p.latitude,p.longitude]),{color:"#2563EB",weight:isFocus?6:3.5,opacity:isFocus?1:0.75}).addTo(map);
    ...
    visible.forEach(loc=>{
      ...
      const iconHtml=`<div style="position:relative;display:inline-flex;flex-direction:column;align-items:center">
        ...
        <div style="position:relative;width:30px;height:30px;overflow:hidden;flex-shrink:0">
          ...
          ${isSup?`<div style="position:absolute;top:-4px;right:-4px;background:#6D28D9;color:white;border-radius:50%;width:13px;height:13px;display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:900;border:1px solid white">★</div>`:""}
        </div></div>`;
          ...
          <div style={{flex:1,overflowY:"auto",padding:"12px 14px"}}>
            ...
            {!tripLoading&&trips.length>0&&(<>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:12}}>
                <div style={{background:"#DBEAFE",borderRadius:10,padding:"7px 6px",textAlign:"center"}}><div style={{fontWeight:800,fontSize:12,color:"#1D4ED8"}}>{fmtDur(totalMove)}</div><div style={{fontSize:9,color:"#1D4ED8"}}>{ar?"حركة":"Move"}</div></div>
                ...
                {trips.map((t,i)=>{
                  ...
                  return(
                    <div key={i} onClick={()=>setSelectedTripIdx(isSel?null:i)}
                      style={{padding:"8px 10px",background:isSel?(t.type==="move"?"#EFF6FF":"#FFFBEB"):"var(--bg)",borderRadius:10,borderLeft:`3px solid ${t.type==="move"?"#2563EB":"#D97706"}`,display:"flex",gap:8,cursor:"pointer",transition:"all .15s",boxShadow:isSel?"0 2px 8px rgba(0,0,0,.1)":"none"}}>
                      <span style={{fontSize:15}}>{t.type==="move"?"🚗":"⏸"}</span>
                      <div style={{flex:1}}>
                        <div style={{display:"flex",justifyContent:"space-between"}}>
                          <span style={{fontWeight:700,fontSize:11,color:t.type==="move"?"#2563EB":"#D97706"}}>{t.type==="move"?(ar?"تحرك":"Move"):(ar?"توقف":"Stop")}</span>
