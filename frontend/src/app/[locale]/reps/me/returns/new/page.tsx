    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <section className="card" style={{ padding: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 11, color: "var(--text-muted)" }}>{ar ? "الفاتورة الأصلية" : "Original invoice"}</div><div style={{ fontFamily: "monospace", fontWeight: 800, color: "#364152" }}>{invoice?.invoice_number}</div></div>
          ...
          <div><div style={{ fontSize: 11, color: "var(--text-muted)" }}>{ar ? "المقبوض بسندات القبض" : "Receipt amount"}</div><div style={{ fontWeight: 800, color: "#364152" }}>{fmt(invoice?.paid_amount)} SAR</div></div>
        </div>
      </section>

      <div style={{ background: "#EFF2F5", border: "1px solid #9BBBAD", padding: "10px 13px", fontSize: 12, color: "#23463A" }}>
      ...
      <section className="card" style={{ padding: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 14, paddingTop: 12, borderTop: "1px solid var(--border)", fontWeight: 800 }}><span>{ar ? "إجمالي المرتجع شامل الضريبة" : "Return total incl. VAT"}</span><span style={{ color: "#364152", fontFamily: "monospace" }}>{fmt(totals.total)} SAR</span></div>
