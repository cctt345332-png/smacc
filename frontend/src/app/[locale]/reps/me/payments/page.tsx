...
const METHOD: Record<string, { ar: string; color: string; bg: string }> = {
  ...
  bank_transfer: { ar: "تحويل بنكي", color: "#364152", bg: "#DBEAFE" },
      ...
      {loading ? (
        ...
        <div style={{ padding:40, textAlign:"center" }}>
          ...
          <button onClick={() => setShowModal(true)}
            style={{ padding:"8px 16px", borderRadius:10, border:"none", background:"#364152", color:"white", fontWeight:700, fontSize:13, cursor:"pointer" }}>
            + {ar ? "سند جديد" : "New Receipt"}
          </button>
        </div>
      ) : (
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  {p.invoice_number && (
                    <div style={{ fontSize:11, color:"var(--text-secondary)", marginTop:3 }}>
                      {ar ? "فاتورة:" : "Invoice:"}{" "}
                      <span style={{ fontFamily:"monospace", color:"#364152" }}>{p.invoice_number}</span>
                    </div>
                  )}
                </div>
              </div>
                ...
                {[
                  ...
                  <button key={m.v} type="button" onClick={() => {
                    ...
                    style={{ flex:1, padding:"9px", borderRadius:8, border:"2px solid",
                      borderColor: mode === m.v ? "#364152" : "var(--border)",
                      background: mode === m.v ? "#EFF2F5" : "var(--surface)",
                      color: mode === m.v ? "#364152" : "var(--text-primary)",
              ...
              {mode === "invoice" && (
                <div>
                  ...
                  {form.invoice_id && (() => {
                    ...
                    const rem = Number(inv.total||0) - Number(inv.paid_amount||0);
                    return (
                      <div style={{ marginTop:8, background:"#EFF2F5", borderRadius:10,
              ...
              {mode === "customer" && (
                <div>
                  ...
                  {customerBalance && !loadingBalance && (
                    <div style={{ marginTop:8, background:"#EFF2F5", borderRadius:10, padding:"12px 14px" }}>
                  ...
                  {[
                    ...
                    <button key={m.v} type="button" onClick={() => upd("payment_method", m.v)}
                      style={{ padding:"8px 4px", borderRadius:8, border:"2px solid",
                        borderColor: form.payment_method === m.v ? "#364152" : "var(--border)",
                        background: form.payment_method === m.v ? "#EFF2F5" : "var(--surface)",
                        color: form.payment_method === m.v ? "#364152" : "var(--text-primary)",
              ...
              <div style={{ display:"flex", gap:10 }}>
                ...
                <button onClick={handleSave} disabled={saving}
                  style={{ flex:2, padding:"12px", borderRadius:10, border:"none",
                    background: saving ? "#93C5FD" : "#364152", color:"white", fontSize:14, fontWeight:700, cursor:"pointer" }}>
