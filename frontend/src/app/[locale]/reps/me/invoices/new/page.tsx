...
export default function RepNewInvoicePage(props: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button onClick={handleSubmitForReview} disabled={saving}
              style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#425E7A", color: "white", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {/* بانر workflow */}
      <div style={{ background: "#EDF3F8", border: "1px solid #9BBBAD", borderRadius: 10, padding: "10px 14px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10, fontSize: 12, color: "#1E40AF" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <Link href={`${base}/reps/me/customers?action=new`}
                style={{ padding: "8px 12px", border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg)", fontSize: 12, color: "#425E7A", fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap", display: "flex", alignItems: "center" }}>
                + {ar ? "عميل جديد" : "New"}
              </Link>
            </div>
          </div>
              ...
              {["cash", "credit"].map(type => (
                <button key={type} type="button" onClick={() => handlePaymentTypeChange(type)}
                  style={{
                    ...
                    borderColor: form.payment_type === type ? (type === "cash" ? "#425E7A" : "#D97706") : "var(--border)",
                    background: form.payment_type === type ? (type === "cash" ? "#425E7A" : "#D97706") : "var(--surface)",
        ...
        <div style={{ background: "var(--surface)", borderRadius: 14, border: "1px solid var(--border)", padding: 20, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ borderTop: "2px solid var(--border)", paddingTop: 10, display: "flex", justifyContent: "space-between" }}>
            ...
            <span style={{ fontWeight: 800, fontSize: 18, color: "#425E7A" }}>{fmt(totals.total)} SAR</span>
          </div>
          <button onClick={handleSubmitForReview} disabled={saving}
            style={{ marginTop: 8, width: "100%", padding: "12px", borderRadius: 10, border: "none", background: "#425E7A", color: "white", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
            {saving ? (ar ? "جاري الإرسال..." : "Sending...") : (ar ? "إرسال للمراجعة" : "Send for Review")}
          </button>
          <button onClick={handleSaveDraft} disabled={saving}
      ...
      <div style={{ background: "var(--surface)", borderRadius: 14, border: "1px solid var(--border)", marginBottom: 16, overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={addLine}
            style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg)", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, color: "#425E7A" }}>
              ...
              {lines.map((line, i) => {
                ...
                return (
                  <tr key={i} style={{ borderTop: "1px solid var(--border)" }}>
                    ...
                    <td style={{ padding: "10px 16px", textAlign: "end", fontWeight: 700, color: "#425E7A", verticalAlign: "top", paddingTop: 14 }}>
                      {fmt(c.total)} SAR
                    </td>
                    <td style={{ padding: "10px 8px", verticalAlign: "top" }}>
                      {lines.length > 1 && (
