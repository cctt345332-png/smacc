  ...
  return (
    <div>
        ...
        <div className="card-body">
          <div className="grid-3">
            <div style={{ padding: "16px 20px", background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)" }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 28, color: "#5A187E" }}>{data?.employer_rate ?? 9}%</div>
            </div>
            <div style={{ padding: "16px 20px", background: "#F5F3FF", borderRadius: 10, border: "1px solid #DDD6FE" }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 28, color: "#75617F" }}>{data?.employee_rate ?? 9}%</div>
            </div>
            <div style={{ padding: "16px 20px", background: "linear-gradient(135deg, #EFF6FF, #F5F3FF)", borderRadius: 10, border: "1px solid #BFDBFE" }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 24, color: "#5A187E" }}>{Number(data?.grand_total ?? 0).toLocaleString()} <span style={{ fontSize: 13, fontWeight: 400 }}>{ar ? "ر.س" : "SAR"}</span></div>
            </div>
          </div>
      ...
      <div className="grid-3" style={{ marginBottom: 24 }}>
        ...
        <div className="stat-card">
          <div className="stat-icon" style={{ background: "#F5F3FF", color: "#75617F" }}><IcDollar /></div>
          <div className="stat-content">
        ...
        <div className="stat-card" style={{ background: "linear-gradient(135deg, #EFF6FF, #F5F3FF)", border: "1px solid #BFDBFE" }}>
          <div className="stat-icon" style={{ background: "white", color: "#5A187E" }}><IcShield /></div>
          <div className="stat-content">
            <div className="stat-label">{ar ? "الإجمالي" : "Grand Total"}</div>
            <div className="stat-value" style={{ color: "#5A187E" }}>{Number(data?.grand_total ?? 0).toLocaleString()}</div>
            ...
            <table>
              ...
              <tbody>
                {data.lines.map((line: any, i: number) => (
                  <tr key={i}>
                    ...
                    <td style={{ color: "#5A187E", fontWeight: 600 }}>{Number(line.employer_share || 0).toLocaleString()}</td>
                    <td style={{ color: "#75617F", fontWeight: 600 }}>{Number(line.employee_share || 0).toLocaleString()}</td>
              ...
              <tfoot>
                <tr style={{ background: "#F8FAFC", fontWeight: 700 }}>
                  ...
                  <td style={{ color: "#5A187E", fontWeight: 700 }}>{Number(data?.total_employer ?? 0).toLocaleString()}</td>
                  <td style={{ color: "#75617F", fontWeight: 700 }}>{Number(data?.total_employee ?? 0).toLocaleString()}</td>
