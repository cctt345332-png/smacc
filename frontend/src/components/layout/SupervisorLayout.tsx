...
const NavIcons = {
  ...
  payments: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/><line x1="6" y1="15" x2="10" y2="15"/></svg>,
...
export default function SupervisorLayout({ children, locale }: { children: React.ReactNode; locale: string }) {
  ...
  const navItems = [
    ...
    { key: "payments", icon: NavIcons.payments,  label: ar ? "السندات" : "Receipts", href: `${base}/supervisor/payments` },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      {/* Top Bar */}
      <header style={{ position: "sticky", top: 0, zIndex: 100, background: "#75617F",
          ...
          return (
            <button key={item.key} onClick={() => router.push(item.href)}
              style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center",
                ...
                cursor: "pointer", color: active ? "#75617F" : "var(--text-muted)",
                padding: "6px 0", position: "relative" }}>
              {active && <span style={{ position: "absolute", top: 6, width: 4, height: 4,
                borderRadius: "50%", background: "#75617F" }} />}
