              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--text-secondary)", marginTop: 2, display: "flex", gap: 8 }}>
                  ...
                  {item.tracking_type === "serial" && (
                    <span style={{ color: "#75617F", fontWeight: 600 }}>سيريال</span>
                  )}
                  {item.tracking_type === "quantity" && (
                    <span style={{ color: item.quantity_on_hand > 0 ? "#6F4A84" : "#DC2626" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: isSerial ? "#5B21B6" : "#4B2A5A", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            {value.item_name}
          </div>
          <div style={{ fontSize: 11, color: isSerial ? "#75617F" : "#6F4A84", marginTop: 1 }}>
            {isSerial
              ? (value.serial_number
      ...
      {value.mode === "batch" && purchaseMode && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#6F4A84", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              ...
              {value.new_serials && value.new_serials.length > 0 ? (
                ...
                <div style={{ background: "#F5F3FF", border: "1px solid #C4B5FD", borderRadius: 8, padding: "8px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#75617F" }}>
                      📦 {value.new_serials.length} {ar ? "سيريال جديد" : "new serials"}
                    </span>
                    <button type="button" onClick={() => setShowPurchaseSerialInput(true)}
                      style={{ background: "none", border: "1px solid #75617F", borderRadius: 6, padding: "3px 8px", fontSize: 11, color: "#75617F", cursor: "pointer" }}>
                      {ar ? "تعديل" : "Edit"}
                    </button>
                  </div>
                  <div style={{ marginTop: 4, fontSize: 10, color: "#75617F", fontFamily: "monospace" }}>
                ...
                <button type="button" onClick={() => setShowPurchaseSerialInput(true)}
                  style={{
                    ...
                    background: "#F5F3FF", color: "#75617F", fontWeight: 600, fontSize: 13,
                ...
                <div style={{ background: "#F7F2F8", border: "1px solid #C8AED4", borderRadius: 8, padding: "8px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button type="button" onClick={() => setShowSerialPicker(true)}
                      style={{ background: "none", border: "1px solid #6F4A84", borderRadius: 6, padding: "3px 8px", fontSize: 11, color: "#6F4A84", cursor: "pointer" }}>
                      {ar ? "تعديل" : "Edit"}
                    </button>
                  </div>
